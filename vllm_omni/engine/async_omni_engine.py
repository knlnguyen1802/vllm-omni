...
class AsyncOmniEngine:
    ...
    def _engine_core_control(
        ...
        *,
        ...
        """Send an EngineCore control op to the Orchestrator and wait for results.

        The Orchestrator executes these on its event loop against
        StageEngineCoreClient (AsyncMPClient) sockets. Do not call those clients
        from the HTTP / AsyncOmni loop.
        """
        rpc_id = uuid.uuid4().hex
        msg = EngineCoreControlRequestMessage(
            rpc_id=rpc_id,
            op=op,
            stage_ids=stage_ids,
            timeout=timeout,
        ...
        if transport is None:
            raise RuntimeError("correlated RPC client is not initialized")
        result_msg = transport.execute(
            ("engine_core_control", rpc_id),
            msg,
            timeout=timeout,
            timeout_message=f"engine_core_control({op}) timed out after {timeout} seconds",
            block_on_submit=True,
        )
        if not isinstance(result_msg, EngineCoreControlResultMessage):
            raise RuntimeError(f"unexpected engine_core_control result type: {type(result_msg).__name__}")
        for result in result_msg.results:
            if isinstance(result, dict) and result.get("error"):
                raise RuntimeError(f"engine_core_control({op}) failed: {result['error']}")
        return list(result_msg.results)

    async def _engine_core_control_async(
        self,
        op: Literal[
            "pause_scheduler",
            "resume_scheduler",
        ...
        **kwargs: Any,
        ...
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(None, lambda: self._engine_core_control(op, **kwargs))
    ...
    async def pause_scheduler_async(
        ...
        *,
        ...
        """Pause AR EngineCore schedulers (diffusion stages are skipped)."""
        await self._engine_core_control_async(
            "pause_scheduler",
            stage_ids=stage_ids,
            mode=mode,
            clear_cache=clear_cache,
    ...
    async def resume_scheduler_async(
        self,
        stage_ids: list[int] | None = None,
        *,
        timeout: float | None = None,
    ) -> None:
        """Resume AR EngineCore schedulers (diffusion stages are skipped)."""
        await self._engine_core_control_async(
            "resume_scheduler",
            stage_ids=stage_ids,
            timeout=timeout,
    ...
    async def sleep_async(
        ...
        *,
        ...
        """Sleep AR stages via EngineCore.sleep (pause then offload/discard)."""
        await self._engine_core_control_async(
            "sleep",
            stage_ids=stage_ids,
            level=level,
    ...
    async def wake_up_async(
        ...
        *,
        ...
        """Wake AR stages via EngineCore.wake_up."""
        await self._engine_core_control_async(
            "wake_up",
            stage_ids=stage_ids,
            tags=tags,
    ...
    async def is_scheduler_paused_async(
        self,
        stage_ids: list[int] | None = None,
        *,
        timeout: float | None = None,
    ) -> bool:
        results = await self._engine_core_control_async(
            "is_scheduler_paused",
            stage_ids=stage_ids,
            timeout=timeout,
        )
    ...
    async def is_engine_sleeping_async(
        self,
        stage_ids: list[int] | None = None,
        *,
        timeout: float | None = None,
