...
# ---------------------------------------------------------------------
#                \U0001f447 Test-only subclass: seed pipeline registry \U0001f447
# ---------------------------------------------------------------------
# Production resolves the pipeline via
# ``VllmOmniPipelineBase.get_pipeline_path(architecture)``, which only
# returns non-``None`` if a third-party ``external_lib`` decorated the
# pipeline with ``@VllmOmniPipelineBase.register(...)``. The in-repo
# test pipeline is not decorated, so we plant the entry inside the Ray
# actor process before the registry is queried, then delegate to the
# real ``run_server``. Nothing else is overridden.
# ---------------------------------------------------------------------
